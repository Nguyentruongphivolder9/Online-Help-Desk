...
{
    public sealed class GetRequestStatusHandler :
        IQueryHandler<GetRequestStatusQueries, IEnumerable <RequestStatusDTO>>
    {
        private readonly IUnitOfWorkRepository _repo;
        private readonly IMapper _mapper;

        public GetRequestStatusHandler(IUnitOfWorkRepository repo, IMapper mapper)
        {
            _repo = repo;
            _mapper = mapper;
        }

        public async Task<Result<IEnumerable<RequestStatusDTO>>> Handle(GetRequestStatusQueries request, CancellationToken cancellationToken)
        {
